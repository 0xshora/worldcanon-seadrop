...
async function deployToBase() {
  ...
  try {
    ...
    console.log(
      "OpenZeppelin method failed, trying manual EIP-1967 slot reading..."
    );
    ...
    try {
      ...
      console.log(
        "✅ Successfully retrieved proxy addresses via manual slot reading"
      );
    } catch (manualError) {
  ...
  if (
    ...
    process.env.BASESCAN_API_KEY !== "your-basescan-api-key"
  ) {
    ...
    if (
      imprintAddresses.implementation &&
      !imprintAddresses.implementation.includes("Error")
    ) {
      try {
      ...
      console.log(
        "Skipping Imprint verification (implementation address not available)"
      );
    }
  } else {
    console.log("\nSkipping Basescan verification (BASESCAN_API_KEY not set)");
    console.log(
      "To enable verification, set BASESCAN_API_KEY in your .env file"
    );
  ...
  console.log(
    "- Upgrade Safety: Manual verification required for library compatibility"
  );
  console.log("\n🔗 Deployed Contracts:");
  console.log(
    `Subject: https://sepolia.basescan.org/address/${subjectAddresses.address}`
  );
  console.log(
    `Imprint Proxy: https://sepolia.basescan.org/address/${imprintAddresses.proxy}`
  );
  if (
    imprintAddresses.implementation &&
    !imprintAddresses.implementation.includes("Error")
    ...
    console.log(
      `Imprint Implementation: https://sepolia.basescan.org/address/${imprintAddresses.implementation}`
    );
  }
  console.log(
    `ImprintLib: https://sepolia.basescan.org/address/${imprintLib.address}`
  );
}
