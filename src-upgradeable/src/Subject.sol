...
contract Subject is ERC721, Ownable {
    ...
    function addSubjects(string[] calldata names, uint64 editionNo)
        external
        onlyOwner
    {
        ...
        returns (string memory)
    {
        ...
        string memory imageURI = (m.latestImprintId == 0 ||
            imprintContract == address(0))
            ? _placeholderSVG(name_)
            : IImprintDescriptor(IImprint(imprintContract).descriptor())
                .tokenImage(m.latestImprintId);
        ...
        bytes memory json = abi.encodePacked(
            ...
            Strings.toString(tokenId),
            '"},'
            '{"trait_type":"Latest Imprint ID","value":"',
            Strings.toString(m.latestImprintId),
            '"}'
            '],"image":"',
            imageURI,
            '"}'
    ...
    /*──────────────────────── placeholder SVG ──────────────────────*/
    function _placeholderSVG(string memory txt)
        internal
        pure
        returns (string memory)
    {
    ...
    /*────────────────────────── view ───────────────────────────────*/
    function subjectName(uint256 tokenId)
        external
        view
        returns (string memory)
    {
